<head>
<title>ZeliTech | Ofertas</title>
</head>
<body>
<section data-index="2" class="ofertass">
...
if (!empty($busqueda) && !empty($cat)) {
	$link = Conectarse();
	$query = "SELECT * FROM productos WHERE name like '%$busqueda%' OR descripcion like '%$busqueda%' AND id_categoria = '$cat' AND oferta>0";
	$q = mysqli_query($link, $query);
}elseif(!empty($cat) && empty($busqueda)){
	$link = Conectarse();
	$query = "SELECT * FROM productos WHERE id_categoria = '$cat' AND oferta>0 ORDER BY id DESC";
	$q = mysqli_query($link, $query);
}elseif (!empty($busqueda) && empty($cat)) {
	$link = Conectarse();
	$query = "SELECT * FROM productos WHERE name like '%$busqueda%' OR descripcion like '%$busqueda%' AND oferta>0";
	$q = mysqli_query($link, $query);
}elseif (empty($busqueda) && empty($cat)) {
	$link = Conectarse();
	$query = "SELECT * FROM productos WHERE oferta>0 ORDER BY id DESC";
	$q = mysqli_query($link, $query);
}else{
	$link = Conectarse();
	$query = "SELECT * FROM productos WHERE oferta>0 ORDER BY id DESC";
...
if (!empty($cat)) {
	...
	?>
	<h2>Productos en oferta filtrados por: <?=$rc['nombre_categoria']?></h2>
	<?php
}
...
while($r=mysqli_fetch_array($q)){
	...
	?>
		<div class="producto">
			...
			<del class="precio-ofer"><?=$divisa?><?=number_format((float)$r['price'],2,'.',',')?></del><span class="precio"><?=$divisa?><?=number_format((float)$preciofinal,2,'.',',')?></span>
			<button class="btn btn-warning fa-pull-right" onclick="agregar_carro('<?=$r['id']?>')"><i class="fa fa-shopping-cart"></i></button>
		</div>
